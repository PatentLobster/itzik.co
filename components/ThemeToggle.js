...
export function ThemeToggle({ panelClassName = "-mt-1 mr-2" }) {
  ...
  return (
    <Listbox value={setting} onChange={setSetting}>
      ...
      <Listbox.Options
        className={clsx(
          "absolute top-full right-0 z-50 w-36 overflow-hidden rounded-lg bg-white py-1 text-sm font-semibold text-slate-700 shadow-lg ring-1 ring-slate-900/10 dark:bg-neutral-800 dark:text-slate-300 dark:ring-0 dark:highlight-white/5",
          panelClassName
        )}
      >
        {settings.map(({ value, label, icon: Icon }) => (
          <Listbox.Option key={value} value={value} as={Fragment}>
            {({ active, selected }) => (
              <li
                className={clsx(
                  "flex cursor-pointer items-center overflow-visible py-1 px-2",
...
export function ThemeSelect() {
  ...
  return (
    <div className="mx-2 flex items-center justify-between overflow-y-visible">
      <label
        htmlFor="theme"
      ...
      <div className="relative flex items-center rounded-lg p-2 pr-2 font-semibold text-slate-700 shadow-sm ring-1 ring-slate-900/10 dark:bg-neutral-600 dark:text-slate-200 dark:ring-0 dark:highlight-white/5">
